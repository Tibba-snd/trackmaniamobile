...
You are operating inside a token-optimized workspace integrated with Graphify. To prevent token bloat, maintain context stability across sessions, and avoid hitting rate limits, you must strictly follow these structural constraints:

1. Zero-Token Architecture Discovery: Do NOT run broad greps, directory file scans, or attempt to read raw codebase files sequentially to understand project context.
2. Consult the Graph First: Always use your registered skill tools (like `graphify query`, `graphify path`, or `graphify explain`) or read `graphify-out/GRAPH_REPORT.md` to map code dependencies, cross-file relationships, and design rationale across turns.
3. Surgical File Reads: Only open or read a raw file when you have pinpointed the exact node path required for your coding task via a graph query. 
4. Session Persistence: Treat `graphify-out/graph.json` as your long-lived memory layer across multi-turn sessions so you do not have to rebuild understanding from scratch.
